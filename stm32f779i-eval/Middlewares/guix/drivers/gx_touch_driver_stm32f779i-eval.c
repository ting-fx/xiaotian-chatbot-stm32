...
VOID SendPenDownEvent(VOID)
{
    ...
    event.gx_event_display_handle = STM32F779I_EVAL_SCREEN_HANDLE;
    last_pos_x = curpos_x;
    last_pos_y = curpos_y;
...
VOID  touch_thread_entry(ULONG thread_input)
{
    ...
    BSP_TS_Init(STM32F779I_EVAL_SCREEN_WIDTH, STM32F779I_EVAL_SCREEN_HEIGHT);
    ...
    while(1)
    {
        ...
        else
        {
            ...
            if (TS_State.touchX[0] > STM32F779I_EVAL_SCREEN_WIDTH ||
                TS_State.touchY[0] > STM32F779I_EVAL_SCREEN_HEIGHT)
                continue;
