...
- syscalls.c/sysmem.c: 用于支持 malloc / printf 等 C 库函数
...
```
- 芯片上电后执行的第一段代码（汇编）
- 主要功能：初始化堆栈指针、拷贝 .data 段、清零 .bss 段、设置中断向量表、跳转到 main()

### Drivers

- CMSIS (Cortex Microcontroller Software Interface Standard)
ARM 官方定义的CPU抽象层，不同厂商的内核用同一套接口

- HAL_Driver(Hardware Abstraction Layer)
ST写的外设驱动库，基于CMSIS
